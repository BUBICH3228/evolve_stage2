...
function WorldPanelChange(id, changes, dels)
{
  ...
  for (var k in changes){
    ...
    if (changes[k].layout !== wp.layout){
      ...
      wp.panel._data = wp.data;

      wp.GetData = () => wp.panel._data;
      wp.IsOnEdge = () => wp.panel.OnEdge;
      wp.IsOffScreen = () => wp.panel.OffScreen;
      wp.DeleteWorldPanel = function(pan){ 
        return function(){
          pan.DeleteAsync(0);
          delete panels[k];
        }
...
function PositionPanels()
{
  ...
  for (var k in panels){
    ...
    var sw = DotaHUD.GetScreenWidth();
    var sh = DotaHUD.GetScreenHeight();
    var scale = 1080 / sh;
