...
@registerAbility()
export class creep_curse_of_the_dead_reduced_health extends BaseAbility {
    // Ability properties
    private caster: CDOTA_BaseNPC = this.GetCaster();

    GetIntrinsicModifierName(): string {
        return modifier_creep_curse_of_the_dead_reduced_health.name;
    }
}

@registerModifier()
export class modifier_creep_curse_of_the_dead_reduced_health extends BaseModifier {
    // Modifier properties
    private caster: CDOTA_BaseNPC = this.GetCaster()!;
    ...
    OnDeath(kv: ModifierInstanceEvent): void {
        ...
        const modifier = kv.attacker.AddNewModifier(
            this.caster,
            this.ability,
            modifier_creep_curse_of_the_dead_reduced_health_debuff.name,
            {
...
@registerModifier()
export class modifier_creep_curse_of_the_dead_reduced_health_debuff extends BaseModifier {
    // Modifier properties
    private caster: CDOTA_BaseNPC = this.GetCaster()!;
    private ability: CDOTABaseAbility = this.GetAbility()!;
    private parent: CDOTA_BaseNPC_Hero = this.GetParent() as CDOTA_BaseNPC_Hero;
    reducedHealthPerStack!: number;
    ...
    GetModifierExtraHealthPercentage(): number {
        if (this.reducedHealthPerStack * this.GetStackCount() < -100) {
            return this.reducedHealthPerStack * (this.GetStackCount() - 1);
        }
        return this.reducedHealthPerStack * this.GetStackCount();
    }
    ...
    override OnRefresh(): void {
        ...
        this.reducedHealthPerStack = -1 * this.ability.GetSpecialValueFor("reduced_health_pct_per_stack");
