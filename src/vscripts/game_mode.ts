...
export class GameMode {
    ...
    CreteTeleport() {
        const teleportOnMountain = Entities.FindByName(undefined, "teleport_on_mountain");
        const teleportOnMountainPoint = Entities.FindByName(undefined, "teleport_on_mountain_point");

        const teleportWithMountain = Entities.FindByName(undefined, "teleport_with_mountain");
        const teleportWithMountainPoint = Entities.FindByName(undefined, "teleport_with_mountain_point");
        Timers.CreateTimer(0.05, () => {
            if (teleportOnMountain == undefined || teleportOnMountainPoint == undefined) {
                return 0.05;
            }
            const enemies = FindUnitsInRadius(
                GameSettings.GetSettingValueAsTeamNumber("players_team"),
                teleportOnMountain.GetAbsOrigin(),
                undefined,
                50,
                UnitTargetTeam.BOTH,
                UnitTargetType.ALL,
            ...
            enemies.forEach((target) => {
                FindClearSpaceForUnit(target, teleportOnMountainPoint.GetAbsOrigin(), false);
        ...
        Timers.CreateTimer(0.05, () => {
            if (teleportWithMountain == undefined || teleportWithMountainPoint == undefined) {
                return 0.05;
            }
            const enemies = FindUnitsInRadius(
                GameSettings.GetSettingValueAsTeamNumber("players_team"),
                teleportWithMountain.GetAbsOrigin(),
                undefined,
                50,
                UnitTargetTeam.BOTH,
                UnitTargetType.ALL,
            ...
            enemies.forEach((target) => {
                FindClearSpaceForUnit(target, teleportWithMountainPoint.GetAbsOrigin(), false);
                if (target.IsRealHero()) {
                    CenterCameraOnUnit(target.GetPlayerOwnerID(), target);
                    target.Stop();
                }
